...
class Player:
    ...
    def serve(self, top_layer: np.ndarray, curr_level: np.ndarray, player_idx: int,
        ...
        if self.state < 24:
            ...
            player_to_pass = Player.calculateBestScoop(self, top_layer, curr_level, get_flavors, other_player_list,
                                                       get_served, get_turns_received)
    ...
    def calculateBestScoop(self, top_layer: np.ndarray, curr_level: np.ndarray, get_flavors: Callable[[], List[int]],
                           player_list, get_served: Callable[[], List[Dict[int, int]]],
                                ...
                                for l in range(j, j + 2):
                                    if curr_level[k, l] == top:
                                        if (k, l) not in cells_taken:
                                            score = score + (len(player_preferences) - player_preferences.index(
                                                top_layer[k][l]))
                                            scoops = scoops + 1
